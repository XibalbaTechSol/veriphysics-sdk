...
std::pair<bool, double> MotionVerifier::analyzeTremor(const std::vector<GyroSample>& gyroData) {
    if (gyroData.empty()) return {false, 0.0};
    
    // 1. Resample Magnitude to 50Hz
    double tStart = gyroData.front().timestamp;
    double tEnd = gyroData.back().timestamp;
    double duration = tEnd - tStart;
    if (duration < 0.5) return {false, 0.0}; // Minimal duration
    
    int Fs = 50;
    int numSamples = (int)(duration * Fs);
    if (numSamples < 32) return {false, 0.0}; // Need reasonable number of samples for FFT
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        if (gIdx + 1 >= gyroData.size()) {
             const auto& last = gyroData.back();
             double mag = std::sqrt(last.x*last.x + last.y*last.y + last.z*last.z);
             signal.push_back(mag);
             continue;
        }
        ...
        double mag0 = std::sqrt(p0.x*p0.x + p0.y*p0.y + p0.z*p0.z);
        double mag1 = std::sqrt(p1.x*p1.x + p1.y*p1.y + p1.z*p1.z);
        
        double dt = p1.timestamp - p0.timestamp;
        double alpha = (dt > 1e-9) ? (t - p0.timestamp) / dt : 0.0;
        
        signal.push_back(mag0 * (1.0 - alpha) + mag1 * alpha);
    }
    ...
    // 3. DFT
    int optimalSize = cv::getOptimalDFTSize(signal.size());
    std::vector<double> padded;
    padded.insert(padded.end(), signal.begin(), signal.end());
    padded.resize(optimalSize, 0.0);
    
    cv::Mat dftInput(padded);
    cv::Mat dftInputFloat;
    dftInput.convertTo(dftInputFloat, CV_32F);
    
    cv::Mat dftPlanes[] = {dftInputFloat, cv::Mat::zeros(dftInputFloat.size(), CV_32F)};
    cv::Mat complexI;
    cv::merge(dftPlanes, 2, complexI);
    
    cv::dft(complexI, complexI);
    
    cv::split(complexI, dftPlanes);
    cv::magnitude(dftPlanes[0], dftPlanes[1], dftPlanes[0]);
    cv::Mat magI = dftPlanes[0];
    
    // 4. Analyze Bands
    // Freq resolution = Fs / optimalSize
    double dF = (double)Fs / optimalSize;
    
    double totalEnergy = 0.0;
    double tremorEnergy = 0.0;
    
    // Iterate up to Nyquist (size/2)
    for (int i = 1; i < magI.rows / 2; ++i) {
        double freq = i * dF;
        float power = magI.at<float>(i); // Magnitude
        power = power * power; // Power spectrum
    ...
    if (totalEnergy < 1e-6) return {false, 0.0};
    
    double ratio = tremorEnergy / totalEnergy;
    // Threshold: > 10% energy in tremor band is significant
    // Also require some minimum total energy (not completely still)
    bool isHandheld = (ratio > 0.10) && (totalEnergy > 0.1); 
    
    return {isHandheld, ratio};
}
...
VerificationResult MotionVerifier::verify(const std::string& videoPath, const std::string& gyroCSVPath) {
    ...
    result.causalityScore = std::max(0.0, std::min(100.0, result.score * 100.0));
    
    // Analyze Tremor
    auto tremorResult = analyzeTremor(gyroData);
    result.isHandheld = tremorResult.first;
    result.tremorEnergy = tremorResult.second;
    
    result.success = true;
    result.duration_analyzed = timestamps.back() - timestamps.front();
    
    std::stringstream ss;
    ss << "Analysis complete. Score: " << (int)result.causalityScore 
       << "/100. Handheld: " << (result.isHandheld ? "Yes" : "No");
